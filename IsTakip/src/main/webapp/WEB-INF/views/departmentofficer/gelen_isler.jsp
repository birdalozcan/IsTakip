...
<body>
 ...
 %><b class="caret"></b></a>
		...
		<div id="page-wrapper">
			...
			<%
				if (listincomingjob.size() == 0) {
					out.println("Henüz atanmış bir İş yok.");
				} else {
			%>			
					<%
						...
						for(int i = 0; i< listincomingjob.size(); i++) {
							if (listincomingjob.get(i).getOncelikid() == 0){
								yuksekoncelikli.add(listincomingjob.get(i));
							} else if (listincomingjob.get(i).getOncelikid() == 1) {
								normaloncelikli.add(listincomingjob.get(i));
							} else if (listincomingjob.get(i).getOncelikid() == 2) {
								dusukoncelikli.add(listincomingjob.get(i));
							} else {
								break;
							}
						}
					...
					<% for (int i=0; i<yuksekoncelikli.size(); i++) { %>
						<form action="/IsTakip/departmentofficer/gelenisBilgileri" method="post">
