...
<script type="text/javascript">
	function checkcontrol() {
		...
		if(document.getElementById("chk1").checked == true) {
			answer = confirm("İşin Bittiğinden Emin misiniz!");
			if(answer == false) {
				<% session.setAttribute("jobstate", 0); %>
			}
			if(answer == true) {
				<% session.setAttribute("jobstate", 1); %>
				window.location.href = "/IsTakip/departmentofficer/isBitimi";
			}
		}
	}
</script>
</head>
<body onchange="checkcontrol()">
	<%
 ...
 %><b class="caret"></b></a>
		...
		<div id="printable">
			...
			<div class="col-xs-10 col-xs-offset-1 col-sm-8 col-sm-offset-2 col-md-offset-login" style="margin-left: 150px; margin-top: 0px;">
						<!--	<form action="/IsTakip/uploadfile" method="post" enctype="multipart/form-data">
									Dosya Seç : <input type="file" name="fileName">
									<br>
								...
								<% if(isbean.getIsbitimi() == 0) { %>
									<table>
										...
										<tr>
											<td valign="middle">İş Bitti mi ? </td><td valign="middle" style="padding-top: 5px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="jobstate" id="chk1" value="Evet"/></td><td align="center">Evet</td>
										</tr>	
									</table>
								<%	} %>
								<br><br>
								<form action="/IsTakip/departmentofficer/isSil" method="post">
									<b>
										<font color="red"><% session.setAttribute("isbeanid", isbean.getIsid()); %>
											<button type="submit" class="submit">İŞİ SİL</button>
										</font>
									</b>
								</form>
								<br><br>
								<p><a href="/IsTakip/departmentofficer/atanmis_isler">Geri</a></p>
								<br>
								<form action="/IsTakip/departmentofficer/atanmisisBilgileri" method="post" id="commentform" onsubmit="return myFunction();">	
