...
<body>
	...
	<script type="text/javascript">
		...
		function drawVisualization() {
		  ...
		  var data = google.visualization.arrayToDataTable([
		    ['Personeller', 'Gelen İşler', 'Devam Eden İşler', 'Biten İşler', 'Average'],
		<% for(int i=0; i < liststaff.size(); i++) { %>
		    ['<%=liststaff.get(i).getAd()+" "+liststaff.get(i).getSoyad()%>', <%=liststaff.get(i).getGelenissayisi()%>, <%=liststaff.get(i).getDevamedenissayisi()%>, <%=liststaff.get(i).getBitenissayisi()%>, <%out.print((double)(liststaff.get(i).getGelenissayisi()+liststaff.get(i).getDevamedenissayisi()+liststaff.get(i).getBitenissayisi())/3.0);%>]<%if(i != liststaff.size()-1) {out.print(",");}%>
		<% } %>
		  ]);
 ...
 %><b class="caret"></b></a>
					...
					<ul class="nav navbar-nav side-nav" style="width: 225px; min-height: 600px; background-color: #F3F3F3; margin-top: 0px;">
						...
						<li class="active"><a href="/IsTakip/departmentofficer/personel_istatistikleri"><i class="glyphicon glyphicon-list"></i> Personel İstatistikleri</a></li>
