...
<script type="text/javascript">
	function myFunction() {
		var infostaff = document.getElementById("infostaff").value;
		if(document.getElementById("chk1").checked == false) {
			...
			document.getElementById("infostaff").value = "";
			return true;
		} else if (!(infostaff == null || infostaff == "" || infostaff == "#")) {
			return true;
		}
	}
</script>
...
<body>
	<%	
		...
		if (session.getAttribute("user") != null) {
			...
			if(user.getUyetip_aciklama().equals("Departman Görevlisi")){
				...
				liststaff = (ArrayList<Staff>) request.getAttribute("liststaff");
 ...
 %><b class="caret"></b></a>
					...
					<ul class="nav navbar-nav side-nav" style="width: 225px; min-height: 600px; background-color: #F3F3F3; margin-top: 0px;">
						...
						<li class="active"><a href="/IsTakip/departmentofficer/gelen_isler"><i class="glyphicon glyphicon-envelope"></i> Gelen İşler <span class="badge"><%=listincomingjob.size() %></span></a></li>
		...
		<div id="page-wrapper">
			<h2>Personele İşi Ata</h2>
				<div class="col-xs-10 col-xs-offset-1 col-sm-8 col-sm-offset-2 col-md-offset-login" style="margin-left: 180px;">
					<div align="left"><b>İşin Başlığı : </b><%=isbean.getBaslik() %></div>
						<form method="post" action="/IsTakip/departmentofficer/yonlendir" id="isyonlendir" onsubmit="return myFunction();">
							<br><input placeholder="Personel Giriniz" id="multipleautocomplete" name="multipleautocomplete" type="text"/>
							<input type="hidden" name="infostaff" id="infostaff" />
							<br><table><tr><td><input type="text" name="chk" id="chk1" value="1"/></td><td>&nbsp;Tüm Personelleri Ekle</td></tr></table><br><br>
							<button id="submitbutton" type="submit" style="margin-left: 120px;">Yayınla</button>
						</form>
						<br><br><br><br>
