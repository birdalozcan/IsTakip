...
<body>
 ...
 %><b class="caret"></b></a>
		...
		<div id="page-wrapper">
			...
			<%
				if (listendingjob.size() == 0) {
					out.println("Henüz biten bir İş yok.");
				} else {
			%>			
					
					<% for (int i=0; i<listendingjob.size(); i++) { %>
						<form action="/IsTakip/staff/bitenisBilgileri" method="post">
					 			<table><tr><td><u><font size="3"><button class="submit" type="submit"><a><%=listendingjob.get(i).getBaslik() %><input name="isbean" type="hidden" value="<%=listendingjob.get(i).getIsid()%>"></a></button></font></u></td></tr></table>					    
						</form>
					<% } %>
				<% } %>
				</div>
	...
	<% 
				...
				System.err.println("staff biten isler jsp error");
			}
			} else { %>
